...
if __name__ == "__main__":
    ...
    def objective(trial):
        ...
        for split_idx in range(5):
            ...
            checkpoint_callback = ptl.callbacks.ModelCheckpoint(
                ...
                monitor="val_acc",
                mode="max",
                dirpath=f"lightning_logs/cnn-split-{split_idx}",
            ...
            model = CNNSAModel.load_from_checkpoint(best_model_path)
            model.eval()
    ...
    study = optuna.create_study(
        storage="sqlite:///tuning/dl_optuna_cnn.db",
        study_name=f"CNN",
        direction="maximize",
        load_if_exists=True,
    )
...
def retrain_best_model():
    ...
    model = CNNSAModel(vocab_size=3_000, **CNNSAModel.BEST_PARAMS)
